// src/pages/Reservas/ResetPasswordClientePage.tsx
...
export function ResetPasswordClientePage() {
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      await clienteAuthService.resetPassword(tenant.slug, token, values.password);
      setEstado('exito');
    } catch (error) {
      ...
      <Stack maw={360} w="100%" gap="md">
        ...
        {estado === 'token-invalido' && (
          <>
            ...
            <Anchor component={Link} to={`/reservas/${slug}/olvide-password`}>
              Pedir un nuevo link
            </Anchor>
          </>
        )}
